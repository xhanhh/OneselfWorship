...
import { useMemorialPoster } from '~/composables/useMemorialPoster'
...
const { createPosterBlob } = useMemorialPoster()
...
async function handlePreviewMemorialCard() {
  ...
  try {
    const blob = await createPosterBlob(memorial.value, currentLink.value)
